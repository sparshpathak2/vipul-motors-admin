...
export default function TruevalueFormModal({ isOpen, onClose, model, setModel, setTruevalueModels }: FormModalProps) {
    ...
    // when modal opens with a model → load edit data
    useEffect(() => {
        if (isOpen && model?.id) {
            (async () => {
                try {
                    const response = await getTruevalueModel(model.id);
                    // Convert year → string for input display
                    const data = {
                        ...response.data,
                    ...
                    if (response.data.imageId) {
                        const selectedImage = images.find(img => img.id === response.data.imageId);
    ...
    }, [isOpen, model, images, reset]);

    const handleSelectImage = async (selected: string) => {
        if (!selected) {
        ...
        try {
            ...
            const response = await getTruevalueModel(model?.id as string);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            const payload = {
                ...data,
                year: data.year ? Number(data.year) : null, // convert back
            };

            if (model?.id) {
                await updateTruevalueModel(model.id, payload);
                toast.success("Model updated successfully!");
            } else {
                await createTruevalueModel(payload);
                        ...
                        <div className="relative border rounded-sm overflow-hidden">
                            ...
                            <div className="absolute top-0 left-0 right-0 bg-white flex justify-between p-2">
                                ...
                                <AllImagesModal
                                    ...
                                    onOpenChange={(isOpen) =>
                                        setActiveModelId(isOpen ? (model?.id || "new") : null)
                                    }
